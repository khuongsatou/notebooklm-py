...
- Before merging, confirm the review landed and address it: `gh api repos/<owner>/<repo>/issues/<PR>/comments --jq '.[]|select(.user.login=="claude[bot]").body'` and resolve any inline `claude[bot]` threads like any other bot's.
